...
BaudRateSelect = A0001_Comp_03_ns.class_("BaudRateSelect", select.Select)
DistanceResolutionSelect = A0001_Comp_03_ns.class_("DistanceResolutionSelect", select.Select)
LightOutControlSelect = A0001_Comp_03_ns.class_("LightOutControlSelect", select.Select)
...
async def to_code(config):
    ...
    if distance_resolution_config := config.get(CONF_DISTANCE_RESOLUTION):
        ...
        await cg.register_parented(s, config[CONF_A0001_Comp_03_ID])
        cg.add(A0001_Comp_03_component.set_distance_resolution_select(s))
    if out_pin_level_config := config.get(CONF_OUT_PIN_LEVEL):
        s = await select.new_select(out_pin_level_config, options=["low", "high"])
        await cg.register_parented(s, config[CONF_A0001_Comp_03_ID])
        cg.add(A0001_Comp_03_component.set_out_pin_level_select(s))
    if light_function_config := config.get(CONF_LIGHT_FUNCTION):
        s = await select.new_select(
            light_function_config, options=["off", "below", "above"]
        )
        await cg.register_parented(s, config[CONF_A0001_Comp_03_ID])
        cg.add(A0001_Comp_03_component.set_light_function_select(s))
    if baud_rate_config := config.get(CONF_BAUD_RATE):
        ...
        await cg.register_parented(s, config[CONF_A0001_Comp_03_ID])
        cg.add(A0001_Comp_03_component.set_baud_rate_select(s))
