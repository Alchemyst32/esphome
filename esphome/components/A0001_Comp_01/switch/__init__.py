...
BluetoothSwitch = A0001_Comp_01_ns.class_("BluetoothSwitch", switch.Switch)
EngineeringModeSwitch = A0001_Comp_01_ns.class_("EngineeringModeSwitch", switch.Switch)
...
async def to_code(config):
    ...
    if engineering_mode_config := config.get(CONF_ENGINEERING_MODE):
        ...
        cg.add(A0001_Comp_01_component.set_engineering_mode_switch(s))
    if bluetooth_config := config.get(CONF_BLUETOOTH):
        s = await switch.new_switch(bluetooth_config)
        await cg.register_parented(s, config[CONF_A0001_Comp_01_ID])
        cg.add(A0001_Comp_01_component.set_bluetooth_switch(s))
